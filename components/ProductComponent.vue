<template>
    ...
    <NuxtLink :to="`/item/${product.id}`">
        <img width="400px" height="400px" class="rounded-t" :src="product.url" alt="product image">
   
    <div id="ProductDetails"  class="bg-white p-1 ">
        ...
        <span class="px-1 relative -top-1.5 text-[#ff6674] tetx-xs font-semibold">Extra 5% off</span>
        ...
        <p class="flex items-center px-1 pt-0.5 text-xs text-[#252525]">
            5,000+ sold <Icon name="material-symbols:star-rate" class="text-[#757575] ml-1.5"/>4.7
        </p>
        <p class="px-1 pt-0.5 text-xs text-[#252525 ] capitalize">
...
const oldPriceComputed = computed( ()=>{
    ...
    return res.toFixed(2) //to make sure we don't get more than 2 decimal places
})
</script>
